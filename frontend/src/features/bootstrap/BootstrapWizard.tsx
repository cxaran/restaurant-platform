...
export function BootstrapWizard({ status }: Readonly<{ status: BootstrapStatusRead }>) {
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <aside className="rounded-3xl border border-white/70 bg-slate-950 p-8 text-white shadow-xl">
        <p className="text-sm font-medium uppercase tracking-[0.28em] text-blue-200">Platform Core</p>
        ...
        <div className="mt-8 grid gap-3 text-sm text-slate-300">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="font-semibold text-white">Administrador fundacional</p>
            <p className="mt-1">Obligatorio, asignado al usuario inicial y con permisos completos definidos por backend.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="font-semibold text-white">Sin sesión automática</p>
...
function StepIndicator({ step }: Readonly<{ step: Step }>) {
  return (
    <ol className="mb-6 grid grid-cols-2 gap-2 text-sm">
      <li className={`rounded-full px-4 py-2 text-center font-medium ${step === "admin" ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"}`}>
        1. Administrador
      </li>
      <li className={`rounded-full px-4 py-2 text-center font-medium ${step === "roles" ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"}`}>
        2. Roles
      </li>
    </ol>
  );
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Estos datos se enviarán al backend para crear el primer usuario activo.</p>
      </div>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">El rol administrador fundacional es obligatorio. Los roles adicionales son opcionales.</p>
      </div>
      <section className="rounded-2xl border border-slate-300 bg-slate-50 p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">System Administrator</p>
            <h3 className="text-lg font-semibold">{draft.system_admin_role.label}</h3>
            <p className="mt-1 text-sm text-slate-600">Asignado al usuario inicial. Permisos completos administrados por backend.</p>
          </div>
          <span className="rounded-full bg-emerald-100 px-3 py-1 text-sm font-medium text-emerald-800">Obligatorio</span>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">{draft.additional_roles.length} de {maxRoles} configurados.</p>
          </div>
          <button type="button" onClick={addRole} disabled={atLimit} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
            Agregar rol
          </button>
        </div>
        {draft.additional_roles.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">No hay roles adicionales. Puedes completar Bootstrap solo con el administrador fundacional.</p>
        ) : null}
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
        <button type="button" onClick={onBack} disabled={pending} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
          Volver
        </button>
  ...
  return (
    <article className="rounded-2xl border border-slate-200 p-4">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={onRemove} className="rounded-md px-3 py-1 text-sm font-medium text-red-700 transition hover:bg-red-50">
          Eliminar
        </button>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="mt-4 flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" className="h-4 w-4 rounded border-slate-300" checked={role.assign_to_initial_user} onChange={(event) => onUpdate({ ...role, assign_to_initial_user: event.target.checked })} />
        Asignar también al administrador inicial
      </label>
      <div className="mt-4 space-y-4">
        {catalogGroups.map((group) => (
          <fieldset key={group.name} className="rounded-xl border border-slate-200 p-3">
            <legend className="px-1 text-sm font-semibold text-slate-800">{group.label}</legend>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              {group.permissions.map((permission) => {
                const id = `role-${role.key}-${permission.access}`;
                return (
                  <label key={permission.access} htmlFor={id} className="flex items-start gap-2 rounded-lg p-2 text-sm text-slate-700 hover:bg-slate-50">
                    <input id={id} type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300" checked={role.permissions.includes(permission.access)} onChange={(event) => togglePermission(permission.access, event.target.checked)} />
                    <span>
